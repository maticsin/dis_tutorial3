...
class DetectRings(Node):
	...
	def synced_callback(self, rgb_msg, pc_msg):
		...
		for le, se in candidates:
			...
			valid, result = self.analyze_ring_candidate(
			...
			ring_type, ring_color_name = result
	...
	def analyze_ring_candidate(self, le, se, center_x, center_y, pc_msg, rgb_image):
		...
		x, y, w, h = cv2.boundingRect(cv2.ellipse2Poly((int(se[0][0]), int(se[0][1])),
		...
		colored_ring_only = cv2.bitwise_and(rgb_image, rgb_image, mask=ring_mask)
		cv2.imshow("Ring pixels", colored_ring_only)
		cv2.waitKey(1)

		rgb_pixels = [(int(p[2]), int(p[1]), int(p[0])) for p in ring_pixels]
		dominant_rgb, _ = Counter(rgb_pixels).most_common(1)[0]
		ring_color_name = self.get_human_readable_color_name(dominant_rgb)

		return True, (ring_type, ring_color_name)
